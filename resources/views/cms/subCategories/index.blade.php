    ...
    <div class="modal fade" id="staticBackdropCreateClientModal" data-bs-backdrop="static" data-bs-keyboard="true"
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <form action="{{ route('cms.subCategories.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-header">
                        <h5 class="modal-title" id="createClientModal">Add Sub Category</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body row">

                        <!-- Sub  Category Title -->
                        <x-input-field :label="' Sub-Category Name'" :name="'title'" :col="12"
                            :required="TRUE" :autofocus="TRUE" />

                        <!-- Foreign category select  -->
                        <x-select-field :label="'Select parent Category'" :placeholder="'----- Select the parent Category -----'" :name="'category_id'" :col="12" :required="TRUE"
    ...
    <script>
        function toggleIsStatus(id) {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: "{{ route('cms.subCategories.toggle.status') }}",
                    data: {
                       'id': id,
                    },
        ...
        $(document).ready(() => {
            ...
            $('#datatable').on('click', '#delete-btn', function(event) {
                ...
                let id = $(this).data('id');
                let url = "{{ route('cms.subCategories.delete', '') }}/" +id;
